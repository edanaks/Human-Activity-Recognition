...
20 test cases given in the assignment correctly, so with stick with this choice.
```{r cache=TRUE}
set.seed(1)
...
We can find the in-sample error by looking at the confusion matrix for training set:
```{r cache=TRUE}
confmatTest
```
